...
function extractMemoryTime(text: string): { time?: string; cleanText: string } {
  ...
  if (match) {
    return { time: match[1], cleanText: text.slice(match[0].length) };
  }
...
function buildSummaryInstruction(memoryMinPerChar = 4, memoryMaxPerChar = 8, userName = '{{user}}'): string {
  ...
  return [
    ...
    '',
    '</think>后直接输出纯JSON，不要输出<content>标签。',
    '',
    '## 输出JSON Schema',
    '',
    '```json',
    ...
    '    {',
    '      "time": "剧情日期+时段",',
    '      "summary": "1-3句话速览（客观白描，禁止修辞比喻）",',
    '      "importance": 4,',
    '      "detail": "完整详情。1级:2-3句 2级:3-4句 3级:4-5句 4级:5-7句 5级:6-8句。起因+经过+结果。保留关键对话原文",',
    '      "characters": ["涉及角色名"],',
    '      "keywords": ["关键词"]',
    '    }',
    '  ],',
    ...
    '',
    '### events[]',
    '- time：从 [时间] 标签提取年/月/日，用阿拉伯数字，精确到日，末尾加时段（晨/上午/午/下午/暮/夜/深夜）。如 "2026年2月5日晨"、"天元243年3月1日夜"',
    '- 如果 [时间] 标签只到日没有时段，根据上下文推断并补充',
    '- 连续事件 time 可以相同，同一时间多条事件不冲突',
    '- characters：只列NPC角色名（从↗已知角色列表选）。{{user}}始终在场，禁止把{{user}}列进去',
    '- importance：1-5整数。5=关键转折/重大告白/世界观揭示，4=关系质变/重要战斗，3=一般互动，2=过渡，1=填充',
    '- keywords：5-10个，选会反复出现在未来对话中的词。优先持久地点/持续物品/关系主题/长期线索。禁止一次性感官描写/路人NPC/一次性动作。物品/功法名用括号注种类（如"灵茶(月华灵茶)"），有简称注括号（如"开窍(开辟空窍)"）。严禁把{{user}}作为关键词',
    '',
    '### characterMemories[]',
    '- characterName：必须用已知角色列表中的正式名称，禁止外貌特征/临时身份',
    '- keywords：5-10个激活词，选会反复出现在未来对话中的词，禁止把{{user}}作为关键词',
    '- memories[].date：与 events[].time 格式相同（年月日+时段，阿拉伯数字），从对应楼层提取',
    `- memories：每个角色${memoryMinPerChar}-${memoryMaxPerChar}条，第一人称`,
    '- 喜欢{{user}}的角色：记忆细节清晰（天气穿着都记得），可美化',
    '- 厌恶{{user}}的角色：记忆有恶意偏差，忽略或扭曲{{user}}的善意',
    '- 中立的角色：非重要事"记不住"或模糊',
    '- 禁止记录NPC→NPC记忆，记忆必须以{{user}}为中心',
    '- 禁止为{{user}}本人创建记忆条目',
    '',
    ...
    '',
    '## 铁律',
    '- 涉及 {{user}} 时，memory content / event detail / summary 中始终用 {{user}}，严禁替换为明月/秋夜等具体人名',
    '- 只整理已有信息，不创作新内容',
    '- 剧情摘要禁止修辞比喻，客观白描',
    '- 角色命名必须用正式名称',
    '- 路人NPC（工具人/一次性出场）不保留',
    `- </think>之后只输出JSON，不要有任何其他文字`,
...
export function parseSummaryJson(outputText: string, summaryVersion: number): ParsedSummary {
  ...
  const characterMemories: CharacterMemory[] = (data.characterMemories || []).map((m: any) => {
    ...
    const getTime = (mem: any) => mem.date || extractMemoryTime(mem.content || '').time;
  ...
  for (const m of memories) {
    ...
    if (orderedAll && orderedAll.length > 0) {
      ...
      for (const core of m.coreMemories || []) {
        parts.push(`- [核心]${typeof core === 'string' ? core : (core as any).text || ''}`);
      }
...
export async function executeGrandSummary(
  ...
  // 提取 </think> 之后的 JSON 输出
  let jsonText = rawResult;
  const thinkingEnd = jsonText.indexOf('</think>');
  if (thinkingEnd !== -1) {
    jsonText = jsonText.slice(thinkingEnd + '</think>'.length);
  }
  // 去掉可能的 <content> 包裹
  const contentMatch = jsonText.match(/<content>([\s\S]*?)<\/content>/i);
  if (contentMatch) {
