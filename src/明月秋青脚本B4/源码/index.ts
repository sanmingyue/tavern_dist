  ...
  eventOn(tavern_events.CHAT_COMPLETION_SETTINGS_READY, async completion => {
    ...
    if (store.settings.memoryActivationEnabled) {
      ...
      if (latestMemory.length > 0) {
        ...
        injectNeuralChain(store, latestMemory, scanText, allNames, characterEntries, userName, sharedQueryEmb ?? undefined, sharedQueryText || undefined);
    ...
    if (store.settings.dynamicProfileEnabled && store.dynamicProfiles.length > 0) {
      ...
      injectDynamicProfiles(store.dynamicProfiles, scanText, Array.from(new Set(allNames)), dpEntries);
    }
    ...
    try {
      ...
      if (summary.timeline && summary.timeline.length > 0) {
        ...
        for (const e of summary.timeline) {
          ...
          parts.push(`[#${eventNum}] [${e.time || '?'}] ${text}`);
        }
        parts.push('');
