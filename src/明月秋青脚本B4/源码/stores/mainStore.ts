...
export const useMainStore = defineStore('main', () => {
  ...
  function updateDynamicProfile(profile: DynamicProfile) {
    ...
    const existing = chatData.value.dynamicProfiles.find(p => p.characterName === profile.characterName);
    if (existing) {
  ...
  function getFusedMemories(
    ...
    // 1. 收集旧窗口的核心（去重，保持首次出现顺序，同时收集 embedding）
    type OldCore = { text: string; embedding?: number[]; time?: string };
    ...
    let oldCoreResult: Array<{ text: string; isCore: boolean }>;
    if (queryEmb && recallOn && oldCores.some(c => c.embedding)) {
      ...
      const scored = oldCores.map(c => {
        ...
        const hybridSim = c.embedding ? hybridW * denseSim + (1 - hybridW) * lexSim : lexSim;
      ...
      const topN = scored.slice(0, Math.max(1, limit));
      ...
      console.info(
        `[智脑-记忆召回] ${characterName}: 远期核心 ${oldCores.length} 条 → 召回 ${topN.length} 条 ` +
          `(混合权重=${(hybridW * 100).toFixed(0)}%语义/${((1 - hybridW) * 100).toFixed(0)}%词汇 有向量:${oldCores.length - withoutEmb} 无向量:${withoutEmb})`,
      );
    } else {
