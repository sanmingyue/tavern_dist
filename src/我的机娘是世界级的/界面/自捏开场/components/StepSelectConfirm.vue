<template>
  <div class="step-page">
    <div class="step-header">
      ...
      <h2 class="step-title">确认搭档</h2>
      <p class="step-desc">确认你选择的机娘，描述你们的开场</p>
    </div>
    ...
    <!-- 选中的机娘信息 -->
    <div v-if="preset" class="summary-card highlight-card">
      <div class="card-header">
      ...
      <div class="card-row">
        ...
        <span class="card-value name-val">{{ preset.name }}</span>
      </div>
      <div class="card-row">
        <span class="card-label">赛车型号</span>
        <span class="card-value">{{ preset.carModel }}</span>
      </div>
      <div class="card-row">
        <span class="card-label">发动机</span>
        <span class="card-value">{{ preset.carType }}</span>
      </div>
      ...
      <div class="stats-grid">
        <div v-for="dim in dimensions" :key="dim" class="stat-item">
          ...
          <div class="stat-bar-track">
            <div class="stat-bar-fill" :style="{ width: preset.stats[dim] + '%' }"></div>
          </div>
          <span class="stat-val">{{ preset.stats[dim] }}</span>
        </div>
      </div>
      ...
      <div class="skill-summary">
        ...
        <div>
          <span class="skill-name">{{ preset.skill.name }}</span>
          <span class="skill-desc">{{ preset.skill.desc }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="state-section">
      <div class="section-header">
        ...
        <div>
          ...
          <p class="state-hint">选择故事开始时的状态</p>
        </div>
      </div>
      <div class="state-options">
        ...
        >
          <input type="radio" :value="state.value" v-model="store.openingState" class="state-radio" />
    ...
    <div class="opening-section">
      <div class="section-header">
        ...
        <div>
          ...
          <p class="opening-hint">描述你与{{ preset?.name || '机娘' }}初次见面的场景，AI 将据此生成开场白</p>
        </div>
      </div>
      <textarea
    ...
    <div class="finish-notice" v-if="!store.submitted">
      ...
      <span>确认后，{{ preset?.name }}的变量信息将写入当前楼层（状态: {{ store.openingState }}）。AI 将生成你们的开场故事。</span>
    </div>
    ...
    <div class="success-notice" v-if="store.submitted">
      ...
      <span>「{{ preset?.name }}」已成为你的搭档！你可以关闭此界面，开始你们的故事。</span>
    </div>
...
const preset = computed(() => store.selectedPreset);
const dimensions: (keyof FiveDimensions)[] = ['加速度', '极速', '操控', '漂移', '耐久'];
const submitting = ref(false);

const stateOptions = [
  { value: '日常' as const, label: '日常', desc: '日常休闲、探索、交流' },
  { value: '赛前准备' as const, label: '赛前准备', desc: '改装、训练、备战' },
  { value: '比赛中' as const, label: '比赛中', desc: '正式赛事进行中' },
];

const openingPlaceholder = computed(() => {
  const name = preset.value?.name || '机娘';
  return `例：在赛场维修区第一次见到${name}，她正独自坐在驾驶舱里……\n或：车队经理把${name}的资料推到你面前，说"她是你的新搭档"……\n或：深夜的练习场，你听到引擎声循声过去，看到${name}正一个人跑弯道……`;
});

async function handleSubmit() {
  submitting.value = true;
  try {
    await store.submitPreset();
  } finally {
