...
function buildDreamtalkInstruction(playStyle?: string): string {
  ...
  return [
    ...
    '4. 角色互动模式：按具体情境分析{{user}}与每个角色的互动习惯。',
    ...
    '- 不抢话党禁止输出性格调色盘部分',
    '- 抢话党基础信息必须每项附带禁止说明',
    '- 基础信息继承：输入中如有"上次分析的基础信息"，近期楼层无变化的部分原样保留，仅更新有变化的条目',
  ].join('\n');
}
...
function buildDreamtalkMaterial(
  ...
  oldUserInfo?: DreamtalkUserInfo,
  oldCharacterNames?: string[],
): string {
  const parts: string[] = [];

  // 已知角色列表：防止 AI 用别名创建重复角色标签
  if (oldCharacterNames && oldCharacterNames.length > 0) {
    parts.push('## 已知角色列表（AI必须使用以下正式名称，禁止自创别名做标题）');
    for (const name of oldCharacterNames) {
      parts.push(`- ${name}`);
    }
    parts.push('');
  }

  // 上次分析的基础信息（AI 继承更新，避免长期信息丢失）
  if (oldUserInfo) {
    const oldParts: string[] = [];
    if (oldUserInfo.basic && oldUserInfo.basic !== '待观察') oldParts.push(`基本信息: ${oldUserInfo.basic}`);
    if (oldUserInfo.appearance && oldUserInfo.appearance !== '待观察') oldParts.push(`外貌特征: ${oldUserInfo.appearance}`);
    if (oldUserInfo.background && oldUserInfo.background !== '待观察') oldParts.push(`背景设定: ${oldUserInfo.background}`);
    if (oldUserInfo.relationship && oldUserInfo.relationship !== '待观察') oldParts.push(`关系设定: ${oldUserInfo.relationship}`);
    if (oldParts.length > 0) {
      parts.push('## 上次分析的基础信息（继承更新，没变化就原样输出）');
      parts.push(oldParts.join('\n'));
      parts.push('');
    }
  }
...
export async function executeDreamtalkAnalysis(
  ...
  const instruction = buildDreamtalkInstruction(playStyle || undefined);
  // 已知角色列表：从旧梦呓中提取，防止 AI 用别名创建重复标签
  const oldCharNames = oldDreamtalk?.characterInteractions?.map(ci => ci.characterName) || [];
  // 只传基础信息（userInfo）和角色名列表，不传旧梦呓的完整内容
  const inputMaterial = buildDreamtalkMaterial(userInputs, userPersonaRaw, oldDreamtalk?.userInfo ?? undefined, oldCharNames);
