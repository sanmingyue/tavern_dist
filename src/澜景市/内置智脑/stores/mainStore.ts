...
export const LANJING_ZHINO_CHAT_DATA_KEY = 'lanjing_zhino_chat_data';
export const LANJING_ZHINO_SETTINGS_KEY = 'lanjing_zhino_settings';
const BACKUP_CHAT_DATA_KEY = 'lanjing_zhino_chat_backups';
/** 澜景内置智脑的稳定备份 ID，不读取原明月秋青脚本的数据。 */
const STABLE_ID = 'lanjing-zhino-data';
/** 仅作迁移兜底的浏览器缓存 key，不能压过酒馆变量。 */
const SETTINGS_LOCAL_KEY = 'lanjing_zhino_settings_cache';

function hasObjectKeys(value: unknown): value is Record<string, any> {
  return Boolean(value) && typeof value === 'object' && Object.keys(value as Record<string, any>).length > 0;
}

function isChatDataShape(value: unknown): value is Partial<ChatData> {
  return Boolean(
    value &&
      typeof value === 'object' &&
      ('summaries' in (value as Record<string, unknown>) ||
        'capturedContents' in (value as Record<string, unknown>) ||
        'userInputRecords' in (value as Record<string, unknown>)),
  );
}

function readCurrentChatData(raw: unknown, currentChatId: string): { data: unknown | null; migrated: boolean } {
  if (!raw) return { data: null, migrated: false };
  if (isChatDataShape(raw)) return { data: raw, migrated: false };
  if (typeof raw === 'object' && raw !== null && currentChatId in (raw as Record<string, unknown>)) {
    console.info('[澜景智脑] 检测到旧 Record<chatId, ChatData> 格式，迁移为当前聊天变量内单份数据');
    return { data: (raw as Record<string, unknown>)[currentChatId], migrated: true };
  }
...
function loadSettingsFromLocal(): any | null {
  ...
  } catch (e) { console.warn('[澜景智脑] 加载设置兜底缓存失败:', e); }
  return null;
}
...
function saveSettingsToLocal(data: any): void {
  ...
  } catch (e) { console.warn('[澜景智脑] 保存设置兜底缓存失败:', e); }
}

function tryReadData(currentScriptId: string, currentChatId: string): { chatData: any; settings: any; migrated: boolean } {
  const chatVariables = getVariables({ type: 'chat' }) ?? {};
  const scriptVariables = getVariables({ type: 'script', script_id: currentScriptId }) ?? {};
  const stable = getVariables({ type: 'script', script_id: STABLE_ID }) ?? {};

  // 设置优先级：澜景脚本变量 > 稳定备份 > 浏览器兜底缓存 > 空。
  let settings = scriptVariables[LANJING_ZHINO_SETTINGS_KEY] ?? stable[LANJING_ZHINO_SETTINGS_KEY] ?? null;
  if (!settings) {
    const localSettings = loadSettingsFromLocal();
    if (localSettings) {
      settings = localSettings;
      console.info('[澜景智脑] 从浏览器兜底缓存迁移设置');
    }
  }

  const directChat = readCurrentChatData(chatVariables[LANJING_ZHINO_CHAT_DATA_KEY], currentChatId);
  if (directChat.data) {
    console.info('[澜景智脑] 从当前聊天变量加载数据');
    return { chatData: directChat.data, settings: settings ?? {}, migrated: directChat.migrated };
  }

  const backupChat = readCurrentChatData(stable[BACKUP_CHAT_DATA_KEY], currentChatId);
  if (backupChat.data) {
    console.info('[澜景智脑] 当前聊天变量为空，从稳定备份恢复数据');
    return { chatData: backupChat.data, settings: settings ?? {}, migrated: true };
  }

  if (hasObjectKeys(settings)) {
    console.info('[澜景智脑] 使用已有设置并初始化当前聊天数据');
  }
  return { chatData: {}, settings: settings ?? {}, migrated: false };
}
...
export const useMainStore = defineStore('main', () => {
  ...
  // ========== 数据加载（主存储 → 跨版本备份回退） ==========
  const { chatData: rawChatData, settings: rawSettings, migrated: migratedFromOld } = tryReadData(currentScriptId, currentChatId);
  const scriptData = ref<ScriptSettings>(ScriptSettingsSchema.parse(rawSettings ?? {}));
  const chatData = ref<ChatData>(ChatDataSchema.parse(rawChatData ?? {}));

  function persistStableBackup(): void {
    const stable = getVariables({ type: 'script', script_id: STABLE_ID }) ?? {};
    const backups = hasObjectKeys(stable[BACKUP_CHAT_DATA_KEY]) ? { ...stable[BACKUP_CHAT_DATA_KEY] } : {};
    backups[currentChatId] = klona(chatData.value);
    replaceVariables(
      {
        ...stable,
        [BACKUP_CHAT_DATA_KEY]: backups,
        [LANJING_ZHINO_SETTINGS_KEY]: klona(scriptData.value),
      },
  ...
  function persistChat(): void {
    ...
    replaceVariables(
      {
        ...variables,
        [LANJING_ZHINO_CHAT_DATA_KEY]: klona(chatData.value),
      },
      { type: 'chat' },
    );
    persistStableBackup();
  ...
  function persistSettings(): void {
    const variables = getVariables({ type: 'script', script_id: currentScriptId }) ?? {};
    replaceVariables(
      {
        ...variables,
        [LANJING_ZHINO_SETTINGS_KEY]: klona(scriptData.value),
      },
      { type: 'script', script_id: currentScriptId },
    );
  ...
  if (migratedFromOld) {
    doPersist();
    console.info('[澜景智脑] 数据已写回澜景内置命名空间');
  }
  ...
  function setActivePersona(id: string) {
    if (id && !scriptData.value.personas.some(p => p.id === id)) return;
    scriptData.value.activePersonaId = id;
    persistSettings();
  }
  ...
  function captureFloorZero() {
    ...
    try {
      ...
      const content = getFinalStoryTextFromRaw(aiMessages[0].message || '', { messageId: 0, source: 'ai_output' });
      if (content) {
        chatData.value.capturedContents.push({
          messageId: 0,
          content,
  ...
  async function loadHistoryFloors(): Promise<number> {
    ...
    for (const msg of aiMessages) {
      ...
      const extractedContent = getFinalStoryTextFromRaw(msg.message, { messageId: msg.message_id, source: 'ai_output' });
      ...
      if (userMsg && !recordIds.has(userMsg.message_id)) {
        newRecords.push({
        ...
        recordIds.add(userMsg.message_id); // 防止同一条被重复添加
      }
    }
