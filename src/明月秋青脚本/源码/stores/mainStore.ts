...
export const useMainStore = defineStore('main', () => {
  ...
  async function rerankEnhancedRecall(
    ...
    for (const name of characterNames) {
      const raw = getFusedMemories(name, undefined, undefined, limit * candidateMult, queryText);
      if (raw.length > limit) {
        candidateMap.set(name, raw);
      } else {
        result.set(name, raw); // 候选不足，直接使用
      }
    }
    ...
    try {
      ...
      for (const [name, candidates] of candidateMap) {
        ...
        if (scored && scored.length > 0) {
          ...
          result.set(name, candidates.slice(0, limit));
        }
      }
      console.info(`[智脑-Rerank] 批量重排完成: ${candidateMap.size}角色, ${charTexts.length}候选`);
    } catch (e) {
