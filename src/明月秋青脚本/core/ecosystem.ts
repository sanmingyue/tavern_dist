...
function buildEcosystemInstruction(
  ...
  const parts: string[] = [
    ...
    '',
    '你必须先在<think></think>中简短分析，然后在<content>标签内输出结果。',
    '',
    '## 两阶段推演规则',
    '',
  ...
  parts.push(
    ...
    '- 严禁分析或推演 {{user}}（主角），主角是玩家控制的角色，不是 NPC',
  ...
  if (isManual) {
    ...
    for (const name of manualChars) {
      ...
      found.push({
        ...
        keywords: [],
        coreMemories: wbContent ? [wbContent.slice(0, 200)] : [],
        recentMemories: [],
      } as CharacterMemory);
    }
    ...
    for (const name of worldBookNames) {
      ...
      worldBookChars.push({
        ...
        attitude: '未知',
        keywords: [],
        coreMemories: [wbContent.slice(0, 300)],
        recentMemories: [],
      } as CharacterMemory);
    }
  }
