...
export function buildNeuralChainInjection(
  store: { getFusedMemories: (characterName: string) => Array<{ text: string; isCore: boolean }> },
  ...
  for (const memory of relevantMemories) {
    ...
    const fused = store.getFusedMemories(memory.characterName);
    ...
    for (let i = 0; i < relevantMemories.length; i++) {
      for (let j = i + 1; j < relevantMemories.length; j++) {
        ...
        function getOrderedTexts(mem: CharacterMemory): string[] {
          const fusedCross = store.getFusedMemories(mem.characterName);
          if (fusedCross && fusedCross.length > 0) return fusedCross.map(m => m.text);
          return [...(mem.coreMemories || []), ...(mem.recentMemories || [])];
        }
...
export function injectNeuralChain(
  ...
  // 构建注入文本
  const injectionText = buildNeuralChainInjection(store, characterMemories, currentCharacters, userName);
