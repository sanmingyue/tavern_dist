...
import { injectContextReplacement, syncSmallSummaryStatus } from './core/contextReplacement';
...
import { archiveItemsFromEvents } from './core/itemMemory';
...
$(() => {
      ...
      if (content) {
        ...
        if (store.settings.captureEnabled) {
          ...
          enqueueAnalysis('small_summary', async () => {
            ...
            const record = await executeSmallSummary(userText, content, userFloor, aiFloor, allNames);
  ...
  eventOn(tavern_events.CHAT_COMPLETION_SETTINGS_READY, async completion => {
    ...
    // --- 摘要替代层：隐藏楼层插入对应小总结 ---
    if (store.chatData.smallSummaries && store.chatData.smallSummaries.length > 0) {
      const hiddenFloors = getHiddenFloorsFromChat();
      const hiddenIds = new Set(hiddenFloors.map((f: any) => f.messageId));
      if (hiddenIds.size > 0) {
        syncSmallSummaryStatus(store.chatData.smallSummaries, hiddenIds);
        injectContextReplacement(completion.messages, store.chatData.smallSummaries, hiddenIds);
      }
    }
    ...
    if (store.settings.dynamicProfileEnabled) {
      const latestCaptured = store.capturedContents[store.capturedContents.length - 1];
      const scanText = latestCaptured?.content || '';
      const allNamesDP = [...store.getAllCharacterNames(), ...store.getDreamtalkCharacterNames()];
  ...
  async function executeSummaryChain(store: ReturnType<typeof useMainStore>) {
    ...
    try {
      ...
      const { summary, nsfwMemories } = await executeGrandSummary(pendingContents, previousSummary, store.settings.memoryMinPerChar, store.settings.memoryMaxPerChar, userGuidance, store.getUserName());
      ...
      // ★ 大总结后触发角色记忆独立更新（V2链路）
      enqueueAnalysis('character_memory_update', async () => {
        try {
          const existingMemories = store.getLatestSummary()?.characterMemories || [];
          const result = await executeCharacterMemoryUpdate(
            pendingContents,
            existingMemories,
          ...
          // 将新角色记忆合并到最新 delta
          const latestDelta = store.getLatestDelta();
          if (latestDelta && result.characterMemories.length > 0) {
            latestDelta.characterMemories = result.characterMemories;
            store.forcePersist();
            console.info(`[智脑-V2链] 角色记忆已更新: ${result.characterMemories.length} 角色`);
          }
          ...
          console.error('[智脑-V2链] 角色记忆更新失败:', e);
        }
      });
      ...
      enqueueAnalysis('dynamic_profile_v2', async () => {
        try {
          ...
          store.forcePersist();
          console.info(`[智脑-V2链] 动态人设V2已更新: ${result.profiles.length} 角色`);
        } catch (e) {
          console.error('[智脑-V2链] 动态人设V2更新失败:', e);
        }
      });
    } catch (error: any) {
