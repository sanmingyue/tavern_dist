...
`只生成结束楼层` 只处理结束楼层，且结束楼层必须是 AI 楼层；`批量生成起止范围` 会先用酒馆助手官方范围读取方式获取 `0-{{lastMessageId}}` 的真实楼层，再按起止楼层和 AI 角色筛选目标。玩家楼层不会被当作出图目标，只会在开启 `上下文包含玩家楼层` 时进入剧情上下文。每楼张数可以设置为 1 到 10，多张会按单图请求排队生成并逐张追加到楼层下方。超过免费范围的尺寸、步数或 SMEA 仍会触发 Anlas 提醒/阻止策略。范围生成完成后，日志会汇总本次处理 AI 楼层数和总图片数；如果没有扫描到 AI 楼层，日志详情会列出实际读取到的楼层和角色。
