...
- 解析后的参数
- seed、模型、尺寸
- 图片缓存 id，多张连续生图时会保存多条
- 缓存过期时间

如果 7 天后图片被脚本清理，或浏览器提前清理缓存，楼层下方会显示缓存已过期，并保留 `重新生成` 按钮。

`保存图片`
按钮会优先尝试系统分享；不支持分享时改为浏览器下载。连续生成多张时，楼层下方会按生成顺序排列图片，并分别显示保存按钮。iOS 上如果分享或下载被 iframe 限制，仍可以长按图片走系统保存或分享。
...
`只生成结束楼层` 只处理结束楼层，且结束楼层必须是 AI 楼层；`批量生成起止范围` 会先用酒馆助手官方范围读取方式获取 `0-{{lastMessageId}}` 的真实楼层，再按起止楼层和 AI 角色筛选目标。玩家楼层不会被当作出图目标，只会在开启 `上下文包含玩家楼层` 时进入剧情上下文。每楼分镜数可以设置为 1 到 10；大于 1 时，漫画模型会把当前目标楼层正文按剧情进度拆成多个连续片段，分别返回多个 `<nai-image>` 块，脚本再按分镜逐张送 NAI 生成并追加到楼层下方，不会拿同一个 tag 块重复抽多张。超过免费范围的尺寸、步数或 SMEA 仍会触发 Anlas 提醒/阻止策略。范围生成完成后，日志会汇总本次处理 AI 楼层数和总图片数；如果没有扫描到 AI 楼层，日志详情会列出实际读取到的楼层和角色。
