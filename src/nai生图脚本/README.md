...
- `接口`：填写 NovelAI Persistent API Token，设置生图接口和订阅测试接口，并查看最新一次日志。
...
`测试账号` 会访问 NovelAI 订阅接口，用来确认 token 是否可用，一般不涉及生图消耗。

`测试生图`
会用 512x512、28 步的轻量参数实际请求一次生图。是否消耗 Anlas 以 NovelAI 账号与官方规则为准；如果担心消耗，可以先把策略设为
`免费优先`。
